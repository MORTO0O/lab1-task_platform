...
class PriorityDescriptor:
    ...
    def __set__(self, instance, value):
        if not isinstance(value, int) or not (self.min_priority <= value <= self.max_priority):
            raise InvalidPriorityError(f"Приоритет должен быть от {self.min_priority} до {self.max_priority}")
